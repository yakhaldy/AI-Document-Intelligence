import { useEffect, useState } from 'react'
import { useAuth } from '../hooks/useAuth'
import { ApiError, listDocuments } from '../api'
import { docTypeLabel } from '../docTypes'

const PAGE_SIZE = 20

const BADGE_CLASS = {
  invoice: 'badge-paid',
  contract: 'badge-unpaid',
  report: 'badge-unpaid',
}

export default function Documents() {
  const { token } = useAuth()
  const [page, setPage] = useState(1)
  const [docType, setDocType] = useState('')
  const [data, setData] = useState(null)
  const [error, setError] = useState(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    let cancelled = false
    async function load() {
      setLoading(true)
      setError(null)
      try {
        const res = await listDocuments(token, { page, pageSize: PAGE_SIZE, docType: docType || undefined })
        if (!cancelled) setData(res)
      } catch (err) {
        if (!cancelled) setError(err instanceof ApiError ? err.message : 'Chargement impossible')
      } finally {
        if (!cancelled) setLoading(false)
      }
    }
    load()
    return () => {
      cancelled = true
    }
  }, [token, page, docType])

  const totalPages = data ? Math.max(1, Math.ceil(data.total / PAGE_SIZE)) : 1

  return (
    <div>
      <h1>Documents</h1>
      <p style={{ color: 'var(--color-text-muted)' }}>
        Tous les documents envoyés, classés automatiquement en facture, contrat ou rapport.
      </p>

      <div style={{ display: 'flex', gap: '0.5rem', margin: '1rem 0' }}>
        <select
          className="input"
          style={{ width: 'auto' }}
          value={docType}
          onChange={(e) => {
            setDocType(e.target.value)
            setPage(1)
          }}
        >
          <option value="">Tous les types</option>
          <option value="invoice">Facture</option>
          <option value="contract">Contrat</option>
          <option value="report">Rapport</option>
        </select>
      </div>

      {error && <p style={{ color: 'var(--color-danger)' }}>{error}</p>}
      {loading && <p style={{ color: 'var(--color-text-muted)' }}>Chargement…</p>}

      {!loading && data && (
        <>
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--color-border)' }}>
                  {['Fichier', 'Type', 'Envoyé le'].map((h) => (
                    <th key={h} style={{ padding: '0.7rem 1rem', fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.items.map((doc) => (
                  <tr key={doc.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    <td style={{ padding: '0.7rem 1rem' }}>{doc.filename}</td>
                    <td style={{ padding: '0.7rem 1rem' }}>
                      <span className={`badge ${BADGE_CLASS[doc.doc_type] ?? 'badge-unpaid'}`}>
                        {docTypeLabel(doc.doc_type)}
                      </span>
                    </td>
                    <td style={{ padding: '0.7rem 1rem' }}>
                      {doc.uploaded_at ? new Date(doc.uploaded_at).toLocaleString('fr-FR') : '—'}
                    </td>
                  </tr>
                ))}
                {data.items.length === 0 && (
                  <tr>
                    <td colSpan={3} style={{ padding: '1rem', textAlign: 'center', color: 'var(--color-text-muted)' }}>
                      Aucun document
                    </td>
                  </tr>
                )}
              </tbody>
            </table>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginTop: '1rem' }}>
            <button className="btn btn-secondary" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>
              Précédent
            </button>
            <span style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
              Page {page} / {totalPages} — {data.total} document{data.total > 1 ? 's' : ''}
            </span>
            <button className="btn btn-secondary" disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)}>
              Suivant
            </button>
          </div>
        </>
      )}
    </div>
  )
}
